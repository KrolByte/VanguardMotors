...
try {
    ...
    try {
        ...
        if ($main_image_id > 0) {
            // CORRECCIÓN AQUÍ: Usamos 'false' en lugar de 0
            $sql_unset_main = "UPDATE vehicle_image SET is_main = false WHERE vehicle_id = :vehicle_id";
            $stmt_unset = $pdo->prepare($sql_unset_main);
            $stmt_unset->execute([':vehicle_id' => $vehicle_id]);   

            // CORRECCIÓN AQUÍ: Usamos 'true' en lugar de 1
            $sql_set_main = "UPDATE vehicle_image SET is_main = true WHERE image_id = :image_id AND vehicle_id = :vehicle_id";
        ...
        if (isset($_FILES['new_images']) && !empty($_FILES['new_images']['name'][0])) {
            ...
            if (!empty($uploaded_new_images)) {
                // CORRECCIÓN AQUÍ: Usamos 'false' en lugar de 0
                $sql_insert_img = "INSERT INTO vehicle_image (vehicle_id, image_url, is_main) VALUES (:vehicle_id, :image_url, false)";
